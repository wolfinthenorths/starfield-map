...
## Закрытая стойка регистрации

- Шкафы для документов (`archiveCabinet`) — закрытый шкаф из **Fallout Tactics / Cupboards** пользовательского набора. Используется зеркальный изометрический ракурс без деформации. Две короткие группы стоят у центрального прямого участка полукруглой стены.
- Служебная калитка (`receptionGate`) и модуль с настольными принадлежностями (`receptionWorktop`) — исходные элементы **bar-table.png**. Стойка собрана на единой сетке, проход за ней сохранён.
- Второй плакат собран из пользовательской **JoinMyParty1.gif**, общей текстуры металлической рамки и типографики. Настенные часы — небольшой пиксельный элемент Canvas. Оба материала закреплены в плоскости стены; ИИ-графика не используется.
